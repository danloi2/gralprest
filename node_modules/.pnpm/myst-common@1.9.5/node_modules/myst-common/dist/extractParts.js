...
export function selectBlockParts(tree, part) {
    ...
    const blockParts = selectAll('block', tree).filter((block) => {
        const blockTags = (block.data?.tags && Array.isArray(block.data.tags) ? block.data.tags : []).map((tag) => tag?.toLowerCase());
        const blockPart = block.data?.part?.toLowerCase();
        return parts
...
function createPartBlock(children, part, opts) {
    const block = { type: 'block', key: createId(), children };
