import { defined, incrementOptions, validateList, validateObjectKeys, validateObject, validateString, validationError, validateBoolean, validateChoice, } from 'simple-validators';
...
export function validateURLEntry(entry, opts) {
    ...
    if (defined(entry.open_in_same_tab)) {
        output.open_in_same_tab = validateBoolean(entry.open_in_same_tab, incrementOptions('open_in_same_tab', opts));
    }
    if (defined(entry.children)) {
        const children = validateList(intermediate.children, incrementOptions('children', opts), (item, ind) => validateEntry(item, incrementOptions(`children.${ind}`, opts)));
        output = { children, ...output };
...
export function validatePatternEntry(entry, opts) {
    ...
    const commonEntry = validateCommonEntry(intermediate, opts);
    const output = { pattern, ...commonEntry };
