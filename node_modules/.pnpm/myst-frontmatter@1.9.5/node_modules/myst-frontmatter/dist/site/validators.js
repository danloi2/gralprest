import { defined, incrementOptions, validateList, validateObject, validateObjectKeys, validateString, validationError, } from 'simple-validators';
...
import { FRONTMATTER_ALIASES, PAGE_KNOWN_PARTS } from './types.js';
...
export function validateSiteFrontmatterKeys(value, opts) {
    ...
    const stashContribAuthors = stash.contributors?.filter((contrib) => stash.authorIds?.includes(contrib.id));
    const stashContribNonAuthors = stash.contributors?.filter((contrib) => !stash.authorIds?.includes(contrib.id));
    if (stashContribAuthors?.length) {
        ...
        const personWithEmail = output.authors?.find((a) => a.email && !a.collaboration && a.corresponding === undefined);
