...
export const includeDirective = {
    ...
    run(data, vfile) {
        ...
        const opts = literal
            ...
            file.split(/\/|\\/).pop())
            : {};
        const caption = literal ? data.options?.caption : undefined;
        const filter = {};
        ...
        if (data.options?.lines) {
            filter.lines = parseLinesString(vfile, select('mystOption[name="lines"]', data.node) ?? undefined, data.options?.lines);
        }
        else {
